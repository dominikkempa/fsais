...
void test(std::uint64_t n_testcases, std::uint64_t max_length) {
  ...
  typedef uint24 block_id_type;
  typedef uint40 block_offset_type;

  // XXX do the same exercise with em_induce_substrings_from_scratch code, e.g., the tests for
  // XXX max capacity type are fucked up.
