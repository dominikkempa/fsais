...
class em_radix_heap {
  ...
  public:
    em_radix_heap(std::uint64_t radix_log, std::string filename, std::uint64_t ram_use) {
      std::uint64_t radix = (1UL << radix_log);
      std::uint64_t depth = ((8UL * sizeof(key_type)) + radix_log - 1) / radix_log;
      std::uint64_t em_queue_count = depth * (radix - 1) + 1;
    ...
    em_radix_heap(std::uint64_t radix_log, std::string filename,
      ...
      // Allocate EM queues.
      std::uint64_t depth = ((8UL * sizeof(key_type)) + m_radix_log - 1) / m_radix_log;
      m_em_queue_count = depth * (m_radix - 1) + 1;
