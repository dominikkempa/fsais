...
void run_tests(std::uint64_t radix_log, std::uint64_t testcases, std::uint64_t max_operations, std::uint64_t bufsize) {
  for (std::uint64_t max_key = 1; max_key <= 250;                 max_key = (max_key * 3 + 1) / 2) test<std::uint8_t>(radix_log, testcases, max_operations, max_key, bufsize);
  for (std::uint64_t max_key = 1; max_key <= 64000;               max_key = (max_key * 3 + 1) / 2) test<std::uint16_t>(radix_log, testcases, max_operations, max_key, bufsize);
  for (std::uint64_t max_key = 1; max_key <= 4000000000;          max_key = (max_key * 3 + 1) / 2) test<std::uint32_t>(radix_log, testcases, max_operations, max_key, bufsize);
  for (std::uint64_t max_key = 1; max_key <= 1000000000000;       max_key = (max_key * 3 + 1) / 2) test<uint40>(radix_log, testcases, max_operations, max_key, bufsize);
  for (std::uint64_t max_key = 1; max_key <= 100000000000000;     max_key = (max_key * 3 + 1) / 2) test<uint48>(radix_log, testcases, max_operations, max_key, bufsize);
  for (std::uint64_t max_key = 1; max_key <= 9000000000000000000; max_key = std::max(max_key + 1, (max_key / 2) * 3))
    test<std::uint64_t>(radix_log, testcases, max_operations, max_key, bufsize);
}

void run_tests_with_given_radix_log(std::uint64_t radix_log, std::uint64_t bufsize) {
  run_tests(radix_log, 1000,  10, bufsize);
  run_tests(radix_log, 1000, 100, bufsize);
  run_tests(radix_log, 100, 1000, bufsize);
}

void run_tests_with_given_radix_log2(std::uint64_t radix_log, std::uint64_t bufsize) {
...
int main() {
  ...
  for (std::uint64_t bufsize = 0; bufsize <= (1UL << 20); bufsize = std::max(bufsize + 1, (bufsize * 3 + 1) / 2)) {
    ...
    for (std::uint64_t radix_log = 8; radix_log <= 12; ++radix_log)
      run_tests_with_given_radix_log2(radix_log, bufsize);
  }
