...
void induce_plus_star_substrings(const chr_t *text, std::uint64_t text_length,
    std::uint64_t ram_use, std::string minus_star_positions_filename,
      ...
      if (head_pos_uint64 > 0) {
        if (text[head_pos_uint64 - 1] <= head_char) {
          ...
          if (empty_output || diff_plus_substrings != diff_plus_substrings_backup)
            ++diff_written_names;
          // Note the swapped order. This is because next time
          // we read this sequence from the end and we want to
          // be able to peek() for the next text position.
