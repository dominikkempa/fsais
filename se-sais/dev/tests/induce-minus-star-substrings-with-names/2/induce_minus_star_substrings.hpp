...
void induce_minus_star_substrings(const chr_t *text, std::uint64_t text_length,
    ...
    std::string minus_star_substrings_filename, std::uint64_t &total_io_volume,
...
//  fprintf(stderr, "  Radix buffer size = %lu\n", radix_heap_bufsize);
  ...
  minus_writer_type *minus_writer = new minus_writer_type(minus_star_substrings_filename);
...
//  fprintf(stderr, "  Induce: ");
  ...
  bool first_head_minus = true;
  std::uint64_t diff_minus_substrings = 0;
  ...
  std::uint64_t diff_minus_substrings_backup = 0;
    ...
    while (!radix_heap->empty() && (!is_next_plus_substring ||
      ...
      if (!first_head_minus) {
        if (!is_prev_tail_name_defined || is_prev_tail_minus != is_tail_minus ||
          ...
          ++diff_minus_substrings;
      } else ++diff_minus_substrings;
      first_head_minus = false;

      if (head_pos_uint64 > 0) {
        if (text[head_pos_uint64 - 1] >= head_char) {
          radix_heap->push(text[head_pos_uint64 - 1],
              heap_item_type((saidx_t)(head_pos_uint64 - 1), (saidx_t)(diff_minus_substrings - 1), 3));
        } else {
          if (empty_output || diff_minus_substrings != diff_minus_substrings_backup)
            ++diff_written_names;
          minus_writer->write(head_pos);
          minus_writer->write((saidx_t)(diff_written_names - 1));
          empty_output = false;
          diff_minus_substrings_backup = diff_minus_substrings;
        }
      }
