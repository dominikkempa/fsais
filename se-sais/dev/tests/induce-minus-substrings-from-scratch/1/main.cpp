...
void test(std::uint64_t n_testcases, std::uint64_t max_length, std::uint64_t radix_heap_bufsize, std::uint64_t radix_log) {
  ...
  for (std::uint64_t testid = 0; testid < n_testcases; ++testid) {
    ...
    // Both work.
    // induce_plus_substrings<chr_t, saidx_tt>(text, text_length, minus_star_positions_filename, plus_substrings_filename, total_io_volume, radix_heap_bufsize, radix_log);
    induce_plus_star_substrings<chr_t, saidx_tt>(text, text_length, minus_star_positions_filename, plus_substrings_filename, total_io_volume, radix_heap_bufsize, radix_log);

    utils::file_delete(minus_star_positions_filename);
    induce_minus_substrings<chr_t, saidx_tt>(text, text_length, plus_substrings_filename, minus_substrings_filename, total_io_volume, radix_heap_bufsize, radix_log);
