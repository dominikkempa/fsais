...
void test(std::uint64_t n_testcases, std::uint64_t max_length, std::uint64_t ram_use, std::uint64_t buffer_size) {
  ...
  for (std::uint64_t testid = 0; testid < n_testcases; ++testid) {
    ...
    // Both work:
    //induce_plus_substrings<chr_t, saidx_tt>(text, text_length, text_length * sizeof(chr_t) + ram_use,
    //    minus_star_positions_filename, plus_substrings_filename, total_io_volume, buffer_size);
    induce_plus_star_substrings<chr_t, saidx_tt>(text, text_length, text_length * sizeof(chr_t) + ram_use,
        minus_star_positions_filename, plus_substrings_filename, total_io_volume, buffer_size);

    utils::file_delete(minus_star_positions_filename);
    induce_minus_substrings<chr_t, saidx_tt>(text, text_length, text_length * sizeof(chr_t) + ram_use,
        plus_substrings_filename, minus_substrings_filename, total_io_volume, buffer_size);
