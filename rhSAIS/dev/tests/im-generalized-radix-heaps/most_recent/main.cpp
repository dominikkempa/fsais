...
//    fprintf(stderr, "\n\nNEWTEST:\n");
    ...
    std::vector<std::uint64_t> radix_logs;
    {
      ...
      std::uint64_t n_levels = (required_bits + radix_log - 1) / radix_log;
      for (std::uint64_t j = 0; j < n_levels; ++j)
        radix_logs.push_back(radix_log);
#else
      std::uint64_t required_bits = log2_floor(max_key) + 1;
      std::uint64_t cur_bits = 0;
      while (cur_bits < required_bits) {
        std::uint64_t next_radix_log = std::min(9L, utils::random_int64(1L, (std::int64_t)(required_bits - cur_bits)));
        radix_logs.push_back(next_radix_log);
        cur_bits += next_radix_log;
      }
#endif
    }
    heap_type *heap = new heap_type(radix_logs, max_operations);
