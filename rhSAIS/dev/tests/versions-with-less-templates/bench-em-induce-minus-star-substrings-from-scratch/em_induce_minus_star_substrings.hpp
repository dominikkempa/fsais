...
void em_induce_minus_star_substrings_large_alphabet(
  ...
  std::uint64_t is_tail_minus_bit = ((std::uint64_t)std::numeric_limits<extext_block_id_type>::max() + 1) / 2;
  ...
  if ((std::uint64_t)std::numeric_limits<ext_block_offset_type>::max() * 2UL + 1 < max_block_size) {
    fprintf(stderr, "\nError: ext_block_offset_type in em_induce_minus_star_substrings_large_alphabet too small!\n");
  ...
  if ((std::uint64_t)std::numeric_limits<extext_block_id_type>::max() * 4UL + 1 < n_blocks) {
    fprintf(stderr, "\nError: extext_block_id_type in em_induce_minus_star_substrings_large_alphabet too small!\n");
    std::exit(EXIT_FAILURE);
  }
  ...
  typedef async_backward_stream_reader<extext_block_id_type> plus_pos_reader_type;
...
template<typename char_type,
  ...
  typename ext_block_offset_type,
  typename extext_block_id_type>
void em_induce_minus_star_substrings_small_alphabet(
    std::uint64_t text_length,
    std::uint64_t max_block_size,
    ...
    std::uint64_t &total_io_volume) {
  std::uint64_t n_blocks = (text_length + max_block_size - 1) / max_block_size;
  std::uint64_t msb_bit = ((std::uint64_t)std::numeric_limits<extext_block_id_type>::max() + 1) / 2;
  std::uint64_t io_volume = 0;
  ...
  if ((std::uint64_t)std::numeric_limits<ext_block_offset_type>::max() * 2UL + 1 < max_block_size) {
    fprintf(stderr, "\nError: ext_block_offset_type in em_induce_minus_star_substrings_small_alphabet too small!\n");
    std::exit(EXIT_FAILURE);
  }
  if ((std::uint64_t)std::numeric_limits<text_offset_type>::max() + 1 < text_length) {
  ...
  if ((std::uint64_t)std::numeric_limits<extext_block_id_type>::max() * 4UL + 1 < n_blocks) {
    fprintf(stderr, "\nError: extext_block_id_type in em_induce_minus_star_substrings_small_alphabet too small!\n");
    std::exit(EXIT_FAILURE);
  }
  ...
  typedef em_radix_heap<char_type, extext_block_id_type> radix_heap_type;
  radix_heap_type *radix_heap = new radix_heap_type(radix_logs, output_pos_filename, ram_use);
...
void em_induce_minus_star_substrings(
  ...
  if (text_alphabet_size <= 2000000)
    em_induce_minus_star_substrings_small_alphabet<char_type, text_offset_type, ext_block_offset_type, extext_block_id_type>(text_length,
    ...
    em_induce_minus_star_substrings_large_alphabet<char_type, text_offset_type, ext_block_offset_type, extext_block_id_type>(text_length,
        max_block_size, ram_use, last_text_symbol, block_count_target, output_pos_filename, plus_pos_filename,
        plus_count_filename, plus_diff_filename, minus_type_filenames, minus_pos_filenames, symbols_filenames, total_io_volume);
}
...
void em_induce_minus_star_substrings(
  ...
  std::string plus_diff_filename = output_filename + ".tmp" + utils::random_string_hash();
  em_induce_plus_star_substrings<
    char_type,
    text_offset_type,
    extext_block_id_type>(
        text_length,
        max_block_size,
        text_alphabet_size,
        ram_use,
...
void em_induce_minus_star_substrings(
  ...
  std::uint64_t n_blocks = (text_length + max_block_size - 1) / max_block_size;
  if (n_blocks < (1UL << 6))
    em_induce_minus_star_substrings<char_type, text_offset_type, ext_block_offset_type, std::uint8_t>
    ...
    em_induce_minus_star_substrings<char_type, text_offset_type, ext_block_offset_type, std::uint16_t>
      (text_length, text_alphabet_size, max_block_size, ram_use, text_filename, output_filename, total_io_volume);
  else
    em_induce_minus_star_substrings<char_type, text_offset_type, ext_block_offset_type, std::uint64_t>
      (text_length, text_alphabet_size, max_block_size, ram_use, text_filename, output_filename, total_io_volume);
}
...
void em_induce_minus_star_substrings(
  ...
  if (max_block_size < (1UL << 32))
    em_induce_minus_star_substrings<char_type, text_offset_type, std::uint32_t>(text_length,
...
void em_induce_minus_star_substrings(
  ...
  em_induce_minus_star_substrings<char_type, text_offset_type>(text_length, text_alphabet_size,
      max_block_size, ram_use, text_filename, output_filename, total_io_volume);
}
...
void em_induce_minus_star_substrings(
  ...
  em_induce_minus_star_substrings<char_type, text_offset_type>(text_length, text_alphabet_size,
      ram_use, text_filename, output_filename, total_io_volume);
}
